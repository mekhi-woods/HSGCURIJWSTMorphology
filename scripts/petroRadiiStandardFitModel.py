...
class petrosianObject():
    ...
    """
    def __init__(self, ID='None', z=0, pos=(0, 0), SB=[], SBerr=[], radius=[], petroR=0.00):
        self.ID = ID
        self.z = z
        self.pos = pos
        self.SB = SB
    ...
    def display_self(self, data, nRingsDisp=10, crop=150, SBsigma=1):
        ...
        ax3.set_xlabel('Radius [pix]'); ax3.set_ylabel('Intensity [MJy/sr]')
        ax3.legend(loc='upper right')

        plt.show()
        return

def obtain_FITS_info(path, wrkBin='SCI', errBin='ERR'):
    """
    ...
    Output: hdr, header of FITS file [astropy.io.fits.header.Header]
...
def covert_world_to_pix_cords(path, crds, z1, z2, show=False):
    """
...
def quick_plot(data=None, title="Default", map=CMAP, interpolation='antialiased', show=True):
    """
    ...
    plt.imshow(data, origin="lower", cmap=map, interpolation=interpolation, vmin=z1, vmax=z2)
    plt.title(title)
    if show:
        plt.show()
    return
...
def image_segmintation(data, display=True):
    ...
    if display:
        quick_plot(segment_map, title="Segmentation Image", map=segment_map.cmap, interpolation='nearest')
    ...
    quick_plot(segment_map, title='kron apertures', map=segment_map.cmap, show=False)
...
def standard_fit(data, center, nRings=10, rLim=150, showText=True):
    """
...
def calc_petro_radius(radius, SB, sens=0.01):
    """
...
if __name__ == "__main__":
    ...
    hdr, data, err, datcrd, z1, z2 = obtain_FITS_info(path=mainPath,
                                                      wrkBin='SCI',
                                                      errBin='ERR')

    print("Displaying targets against data...")
    targetPixCords, targetIDs, targetZs = covert_world_to_pix_cords(path=targetPath,
                                                                    crds=datcrd,
                                                                    z1=z1, z2=z2,
                                                                    show=True)
    ...
    for i in range(len(targetPixCords)):
        allPetrosians.append(petrosianObject(ID=targetIDs[i],
                                             z=targetZs[i],
                                             pos=(targetPixCords[i][0], targetPixCords[i][1])))

    # crop = 150
    # tempData = data[int(targetPixCords[0][1])-crop:int(targetPixCords[0][1])+crop,
    #                 int(targetPixCords[0][0])-crop:int(targetPixCords[0][0])+crop]
    # temp_x, temp_y, temp_eps, temp_apers = image_segmintation(tempData)
    # plt.imshow(tempData, vmin=z1, vmax=z2, cmap=CMAP)
    # plt.show()
    ...
    for i in range(len(allPetrosians)):
        ...
        radius, SB, SB_err = standard_fit(data=data,
                                          center=allPetrosians[i].pos,
                                          nRings=100,
                                          rLim=150,
                                          showText=False)
        print("["+str(i+1)+'/'+str(len(allPetrosians))+"] Fit finished.\n")
        if len(SB) != 0:
    ...
    allPetroR = []
    for i in range(len(allPetrosians)):
        print("[" + str(i + 1) + '/' + str(len(allPetrosians)) + "] Calculating radius...") # Ennumerates through the loop
        petroR = calc_petro_radius(radius=allPetrosians[i].radius,
                                   SB=allPetrosians[i].SB,
                                   sens=PETRO_SENS)
